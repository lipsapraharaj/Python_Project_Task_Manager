...
class TaskManagerApp:
    ...
    def show_tasks(self):
        ...
        if connection:
            try:
                query = f"SELECT TaskId, description, deadline, status FROM {self.tasks_table} WHERE username = %s"
        ...
        if not self.tasks:
            ...
            # Display only task descriptions without the username
    ...
    def mark_task(self, status):
        ...
        if connection:
            try:
                query = f"UPDATE {self.tasks_table} SET status = %s WHERE TaskId = %s AND username = %s"
                data = (status, self.tasks[index]['TaskId'], self.current_user)
                execute_query(connection, query, data)
                connection.commit()
            except Exception as e:
